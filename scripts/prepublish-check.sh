...
require_file assets/config.js
require_file assets/app.js
require_file assets/styles.css
require_file backend/google-apps-script/Code.gs
...
if ! grep -q 'assets/config.js' index.html; then
  fail 'index.html must load assets/config.js before assets/app.js'
fi

if ! grep -q 'submitResponse' assets/app.js; then
  fail 'assets/app.js does not include the submission handler'
fi
